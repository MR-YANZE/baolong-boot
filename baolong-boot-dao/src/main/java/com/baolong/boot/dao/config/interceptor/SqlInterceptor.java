package com.baolong.boot.dao.config.interceptor;
 ...
 * SQL拦截器
 *
 * @author Baolong 2024年06月15 21:37
 * @version 1.0
 * @since 1.8
 */
@Intercepts({
...
public class SqlInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		try {
			...
			logger.error("SQL语句获取异常", exception);
		} finally {
			// 拼接日志打印过程
			logger.info("SQL语句: {}", printSql);
			logger.info("执行结果: {}", proceed);
			logger.info("SQL耗时：{}ms", (endTime - startTime));
		}
		return proceed;
	}
